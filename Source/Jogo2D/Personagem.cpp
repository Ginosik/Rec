...
UFUNCTION(BlueprintPure, Category = "Life") int APersonagem::GetVidasIniciais()
{
	return UFUNCTION(BlueprintPure, Category = "Life") int();
}

UFUNCTION(BlueprintPure, Category = "Life") int APersonagem::GetVidasAtuais()
{
	return UFUNCTION(BlueprintPure, Category = "Life") int();
}

UFUNCTION(BlueprintCallable, Category = "Life") void APersonagem::UpdateVidasAtuais(int Vidas)
{
	VidasAtuais = VidasAtuais + Vidas;
	return UFUNCTION(BlueprintCallable, Category = "Life") void();
}
...
void APersonagem::Tick(float DeltaSeconds)
{
	...
	UpdateVidasAtuais(DeltaSeconds);

	if (points == 5)
	{
		UE_LOG(LogTemp, Warning, TEXT("Você Passou de Fase!"));
		points++;
	}
